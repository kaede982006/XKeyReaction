...
- Any key — or any mouse click / touch tap — counts on the green "go" phase
- Per-session statistics: last, best, average, round counter
- Layout-independent keyboard input via `KeyboardEvent.code`; mouse, touch,
  and pen input handled via `pointerdown` for low-latency response
...
2. Press any key, click, or tap to begin a session of **10 rounds**.
3. The screen turns **red** — wait.
4. When the screen turns **green** and shows `PRESS`, hit any key, click your
   mouse, or tap the screen as fast as you can.
5. Your reaction time is shown on the **blue** result screen. Press, click,
   or tap to continue to the next round.
6. After 10 successful rounds the **purple** summary screen shows your average
   and best time. Press, click, or tap to start a new session.

### Failure rule

Any input during the red "wait" phase (a false start) — keypress, click, or
tap — resets the session back to **round 1** and clears all stats.
...
Mouse clicks and touch taps anywhere on the page count as a press. Internally
the page listens for `pointerdown`, which fires earlier than a `click` event
and unifies mouse, touch, and pen input. Clicks on the footer license link
are ignored so the link still works normally.
